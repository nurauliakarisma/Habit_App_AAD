...
class RandomHabitActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.priorityLevelHigh.observe(this) { highPriorityList ->
            highPriorityList?.let {
                adapter.submitData(RandomHabitAdapter.PageType.HIGH, it)
            }
        }

        viewModel.priorityLevelMedium.observe(this) { mediumPriorityList ->
            mediumPriorityList?.let {
                adapter.submitData(RandomHabitAdapter.PageType.MEDIUM, it)
            }
        }

        viewModel.priorityLevelLow.observe(this) { lowPriorityList ->
            lowPriorityList?.let {
